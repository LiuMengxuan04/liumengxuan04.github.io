...
| **TUI**              | 状态机可视化：工具态、审批态、折叠摘要等                            | `tui/`*：不只刷最终回答                                                             |
...
| `src/skills.ts` / `src/mcp.ts` / `src/manage-cli.ts` | Skills 发现、MCP 客户端、面向用户的 `minicode mcp` / `minicode skills` |
...
仓库里的开发规范和项目路线图里明确写明了开发规范以及目前亟待实现的功能扩展，十分欢迎认领并成为重要特性的 Contributors~~~  
另外项目后面还有一个比较重要的规划，就是做 Python、Go、Rust 等多语言版本分支，所以也很欢迎对这些方向感兴趣的朋友一起参与，成为 Collaborators，carry某个语言版本～～～